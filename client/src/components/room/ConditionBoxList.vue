...
<template>
  <section class="rounded-xl border border-[#c8d5e8] bg-[#eef4ff] p-4 text-[#121a2a]" aria-label="条件一覧">
    <header class="flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex gap-2">
        ...
        <button type="button" :disabled="disabled || items.length === 0"
    ...
    <ul v-else class="mt-4 grid gap-2" aria-label="追加済みの条件">
      <li v-for="item in items" :key="item.id"
        ...
        <button type="button" :disabled="disabled" :aria-label="`${item.label}を削除`"
