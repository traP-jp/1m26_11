    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    Router::new()
        ...
        .route(
            ...
            get(get_problems::<I, A, E>).post(create_problem::<I, A, E>),
        )
        .route(
        ...
        .route(
            ...
            get(get_problem_assets::<I, A, E>).post(upload_problem_asset::<I, A, E>),
        )
        .route(
...
#[tracing::instrument(skip_all)]
fn get_problem_assets_validation(
    path_params: models::GetProblemAssetsPathParams,
) -> std::result::Result<(models::GetProblemAssetsPathParams,), ValidationErrors> {
    path_params.validate()?;
...
#[tracing::instrument(skip_all)]
async fn get_problem_assets<I, A, E>(
    method: Method,
    TypedHeader(host): TypedHeader<Host>,
    cookies: CookieJar,
    Path(path_params): Path<models::GetProblemAssetsPathParams>,
    State(api_impl): State<I>,
) -> Result<Response, StatusCode>
where
    I: AsRef<A> + Send + Sync,
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    #[allow(clippy::redundant_closure)]
    let validation =
        tokio::task::spawn_blocking(move || get_problem_assets_validation(path_params))
            .await
            .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .get_problem_assets(&method, &host, &cookies, &path_params)
        .await;

    let resp = match result {
        Ok(rsp) => match rsp {
            apis::problems::GetProblemAssetsResponse::Status200 {
                body,
                cache_control,
            } => {
                let mut response = Response::builder();
                if let Some(cache_control) = cache_control {
                    ...
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers
                        .insert(HeaderName::from_static("cache-control"), cache_control);
                }
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
            ...
            apis::problems::GetProblemAssetsResponse::Status400_Room(body) => {
                let mut response = Response::builder();
                let mut response = response.status(400);
                {
                    let mut response_headers = response.headers_mut().unwrap();
            ...
            apis::problems::GetProblemAssetsResponse::Status401(body) => {
                let mut response = Response::builder();
                let mut response = response.status(401);
                {
                    let mut response_headers = response.headers_mut().unwrap();
            ...
            apis::problems::GetProblemAssetsResponse::Status404_Active(body) => {
                let mut response = Response::builder();
                let mut response = response.status(404);
                {
                    let mut response_headers = response.headers_mut().unwrap();
            ...
            apis::problems::GetProblemAssetsResponse::Status409(body) => {
                let mut response = Response::builder();
                let mut response = response.status(409);
                {
                    let mut response_headers = response.headers_mut().unwrap();
            ...
            apis::problems::GetProblemAssetsResponse::Status500_DB(body) => {
                let mut response = Response::builder();
                let mut response = response.status(500);
                {
                    let mut response_headers = response.headers_mut().unwrap();
...
#[tracing::instrument(skip_all)]
fn get_problems_validation(
    path_params: models::GetProblemsPathParams,
) -> std::result::Result<(models::GetProblemsPathParams,), ValidationErrors> {
    path_params.validate()?;
...
#[tracing::instrument(skip_all)]
async fn get_problems<I, A, E>(
    method: Method,
    TypedHeader(host): TypedHeader<Host>,
    cookies: CookieJar,
    Path(path_params): Path<models::GetProblemsPathParams>,
    State(api_impl): State<I>,
) -> Result<Response, StatusCode>
where
    I: AsRef<A> + Send + Sync,
...
{
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || get_problems_validation(path_params))
        .await
        .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .get_problems(&method, &host, &cookies, &path_params)
        .await;

    let resp = match result {
        Ok(rsp) => match rsp {
            apis::problems::GetProblemsResponse::Status200(body) => {
                let mut response = Response::builder();
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
            ...
            apis::problems::GetProblemsResponse::Status400_JSON(body) => {
                let mut response = Response::builder();
                let mut response = response.status(400);
                {
                    let mut response_headers = response.headers_mut().unwrap();
            ...
            apis::problems::GetProblemsResponse::Status401(body) => {
                let mut response = Response::builder();
                let mut response = response.status(401);
                {
                    let mut response_headers = response.headers_mut().unwrap();
            ...
            apis::problems::GetProblemsResponse::Status404_Active(body) => {
                let mut response = Response::builder();
                let mut response = response.status(404);
                {
                    let mut response_headers = response.headers_mut().unwrap();
            ...
            apis::problems::GetProblemsResponse::Status500_Server(body) => {
                let mut response = Response::builder();
                let mut response = response.status(500);
                {
                    let mut response_headers = response.headers_mut().unwrap();
...
#[tracing::instrument(skip_all)]
fn get_room_validation(
    path_params: models::GetRoomPathParams,
) -> std::result::Result<(models::GetRoomPathParams,), ValidationErrors> {
    path_params.validate()?;
...
#[tracing::instrument(skip_all)]
async fn get_room<I, A, E>(
    method: Method,
    TypedHeader(host): TypedHeader<Host>,
    cookies: CookieJar,
    Path(path_params): Path<models::GetRoomPathParams>,
    State(api_impl): State<I>,
) -> Result<Response, StatusCode>
where
    I: AsRef<A> + Send + Sync,
...
{
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || get_room_validation(path_params))
        .await
        .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .get_room(&method, &host, &cookies, &path_params)
        .await;

    let resp = match result {
        Ok(rsp) => match rsp {
            apis::rooms::GetRoomResponse::Status200(body) => {
                let mut response = Response::builder();
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
            ...
            apis::rooms::GetRoomResponse::Status400_JSON(body) => {
                let mut response = Response::builder();
                let mut response = response.status(400);
                {
                    let mut response_headers = response.headers_mut().unwrap();
            ...
            apis::rooms::GetRoomResponse::Status404(body) => {
                let mut response = Response::builder();
                let mut response = response.status(404);
                {
                    let mut response_headers = response.headers_mut().unwrap();
